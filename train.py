...
def train(net, dataset, criterion, optimizer, scheduler, evaluator, train_loader, test_loader, network_name, batch_update, max_iters=2000, save_model=True, log_file=None, method="foo", dest="/data"):
    iteration = 0
    epoch = 0
    flag = False
    best_test_loss = None
    ...
    while True:
        ...
        for i, gt_batch in enumerate(train_loader):
            net.train()
            # gt_batch["img"].shape --> torch.Size([16, 3, 321, 321])
            # 早期的PyTorch版本中，Variable类用于封装张量并支持自动求导,现在已经不需要了
            ...
            if iteration % test_int == 0:
                ...
                with torch.no_grad():
                    ...
                    for i, gt_batch in enumerate(test_loader):
                        ...
                        # preds2 = preds2.argmax(dim=1)


                        # true_label_list.append(gt_batch["img"].cpu().detach().numpy())
                        # pred_label_list.append(preds2.cpu().detach().numpy())
                    ...
                    # print(f"{method}: TEST Task1 ACC on {epoch}th epoch:", accuracy)
                    log_file.write(f"{method}: TEST LOSS on {epoch}th epoch: {test_loss}")
                    # log_file.write(f"{method}: TEST ACC on {epoch}th epoch: {accuracy}")
                    log_file.write("\n")
