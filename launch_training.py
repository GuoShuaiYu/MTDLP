...
if __name__ == "__main__":
    ...
    parser.add_argument('--method', type=str, help='method name: baseline, disparse_static, disparse_pt, disparse_dynamic', default="baseline")
    parser.add_argument('--ratio',type=int, help='percentage of sparsity level', default=90)
    parser.add_argument('--dataset', type=str, help='dataset: choose between nyuv2, cityscapes, taskonomy', default="cityscapes")
    parser.add_argument('--dest', default='/home/guo/DiSpare/data/baseline/save_model', type=str, help='Destination Save Folder.')
    ...
    if method == "disparse_dynamic":
        ...
        net = train(net, dataset, criterion, optimizer, scheduler, evaluator, train_loader, test_loader, network_name, batch_update, max_iters=MAX_ITERS, log_file=log_file, save_model=True, method=method, dest=dest)
    else:
        optimizer = torch.optim.Adam(net.parameters(), lr = RETRAIN_LR, weight_decay = WEIGHT_DECAY)
        net = train(net, dataset, criterion, optimizer, scheduler, evaluator, train_loader, test_loader, network_name, batch_update, max_iters = RETRAIN_EPOCH, save_model=True, log_file=log_file, method=method, dest=dest)
