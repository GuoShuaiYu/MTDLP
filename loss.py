...
class SceneNetLoss(nn.Module):
    def __init__(self, dataset, tasks, tasks_num_class, lambdas, device, data_root=None):
        ...
        if dataset == "taskonomy":
            ...
            #这里的 ignore_index=255 参数是可选参数，表示在计算损失时忽略某些特定标签。
            # 当某个样本的真实标签是 255 时，这个样本不会被计入到总损失的计算中。
            # 这对于处理图像分割或序列标注等任务非常有用，其中有些像素点或者时间步可能没有类别标签或者应当被忽略不计。
            #例如，在语义分割任务中，如果背景像素通常标记为255，那么使用此参数可以确保模型仅针对有实际类别的像素点进行训练，而不考虑背景像素。
        elif dataset == "cityscapes":
            self.cross_entropy = nn.CrossEntropyLoss(ignore_index=-1)
        else:
            print("Unrecocgnized Dataset.")
            exit()
        #nn.CosineSimilarity 是一个用于计算两个向量之间余弦相似度的模块。
    ...
    def forward(self, preds, targets):
        ...
        if 'seg' in self.tasks:
            # self.tasks.index('seg') --> 0   tasks_num_class --> [40, 3]
    ...
    def get_seg_loss(self, seg_num_class):
        # 首先对模型预测出的特征图（seg_pred）进行维度调整，使得每像素的各类别预测概率排列在一起；
        # 然后contiguous确保张量在内存中连续存储；
        # 最后，将调整后的张量reshape成一个二维张量，其中每一行代表一个像素对应的各类别的预测概率，通常在语义分割任务中，每个像素点对应一个类别
        # 方便后续进行损失函数计算或进行argmax操作得到每个像素的预测类别。
        # self.seg_pred.shape[0] 就是当前批次中包含的样本数量
        # self.seg_pred.shape[-2:] 表示 self.seg_pred 张量的倒数第二个维度和最后一个维度。 --> torch.Size([41, 41])
        # F.interpolate(self.seg.float(), size=new_shape)：对 self.seg 进行插值操作以改变其大小至 new_shape。
        # view(-1)：重塑张量为一维形式，它会把所有的元素压平成一个单一的维度 seg_resize.permute(0, 2, 3, 1).contiguous().view(-1)这一系列操作常用于准备数据以便计算像交叉熵这样的损失函数
