...
class SceneNet(nn.Module):
    def __init__(self, num_classes_tasks, skip_layer=0):
        ...
        # setattr（O,K,V）为对象O中的每一个K属性复制一个v
        # 为self对象里面每一个'task%d_fc1_c0' % (t_id + 1)属性 赋值一个Classification_Module(512 * block.expansion, num_class, rate=6)
    ...
    def forward(self, img, num_train_layers=None, hard_sampling=False, mode='train'):
        #img.shape --> torch.Size([16, 3, 321, 321])
        #self.layers --> [3, 4, 6, 3] --> 16
        #self.skip_layer --> 0
        ...
        # 使用共享主干结构，共享提取特征  Use a shared trunk structure. Shared extracted features
        outputs = []
